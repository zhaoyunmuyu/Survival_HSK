...
功能：
- 规范化评论表字段类型（字符串/时间/数值）；
- 按餐厅聚合评论，限制每家餐厅的序列长度；
- 为每条评论构建三类特征：文本向量、图片向量、多维结构化特征（时间段/评分等）；
- 以张量形式缓存，后续 Dataset 取数无需重复计算，显著降低开销。
"""
...
def prepare_review_dataframe(review_df: pd.DataFrame) -> pd.DataFrame:
    """规范评论表的数据类型并派生辅助列（如 `user_location_code`）。"""
...
def build_restaurant_review_cache(
    ...
    *,
    ...
    """以餐厅为单位预计算并缓存评论序列的张量表示（文本/图片/结构化）。"""
    ...
    for restaurant_id, group in tqdm(grouped, desc="prepare_reviews", total=total_groups):  # 进度条名称使用英文避免编码问题
        if group.empty:
            continue
        ...
        for idx, row in enumerate(group.itertuples(index=False)):
            ...
            review_date = row.review_date  # 计算与最早评论之间的月份差，并量化到季度段（1/2/3/...）
    ...
    LOGGER.info(
        "Built restaurant review cache for %d restaurants (missing text: %d, missing images: %d)",
